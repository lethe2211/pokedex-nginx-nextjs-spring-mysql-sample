...
const PokedexList: React.VFC = () => {
  const { loading, error, data } = useQuery<AllPokedexDataResponse>(
    GET_ALL_POKEDEX_DATA_QUERY
  );
      ...
      <div className="container my-12 mx-auto px-4 md:px-12">
        <div className="flex flex-wrap -mx-1 lg:-mx-4">
          {allPokedexData
            ...
            .map((pokedexData, i) => {
              const pokemonIdStr = `#${String(pokedexData.id).padStart(
                3,
                "0"
              )}`;
              const imageUrl = `/images/pokemon/${String(
                pokedexData.id
              ).padStart(3, "0")}.png`;
              const linkUrl = `/pokemon/${String(pokedexData.id).padStart(
                3,
                "0"
              )}`;
