...
const PokedexDetail: React.VFC<Props> = ({ pokemonId }) => {
    const { loading, error, data } = useQuery<PokedexDataResponse>(GET_POKEDEX_DATA_QUERY, { variables: { pokemonId: pokemonId } });
    ...
    const previousPokemonUrl = (pokemonId - 1 >= 1) ? `/pokemon/${String(pokemonId - 1).padStart(3, '0')}` : 'N/A';
    const nextPokemonUrl = (pokemonId + 1 <= 809) ? `/pokemon/${String(pokemonId + 1).padStart(3, '0')}` : 'N/A';
                ...
                <div className='flex justify-center my-3 gap-x-9'>
                    ...
                    <div className='flex flex-col gap-y-9 w-96'>
                        <div className='flex items-center justify-between'>
                            <span className='text-3xl font-bold mr-9'>{name?.capitalizeFirstChar()}</span>
